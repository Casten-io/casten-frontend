...
import { JsonRpcSigner, JsonRpcProvider, StaticJsonRpcProvider, Web3Provider } from '@ethersproject/providers'
import { BENTOBOX_ADDRESS, ChainId, WNATIVE } from '@sushiswap/core-sdk'
import { toShare } from '../lib/bentoBox'
import { toElastic } from '../lib/rebase'
...
export async function signMasterContractApproval(
  ...
  const bentoBoxContract = new ethers.Contract(
    getAddresses(Number(chainId)).BENTOBOX_ADDRESS, 
    BentoBoxContract, 
    library
  )
...
export default class KashiCooker {
    ...
    private library: JsonRpcProvider | StaticJsonRpcProvider
    ...
    constructor(
      ...
      library: JsonRpcProvider | StaticJsonRpcProvider,
      chainId: ChainId | undefined
    ) {
      this.pair = pair
    ...
    async cook() {
      console.log("before executing cook() ...", this.pair.address)
