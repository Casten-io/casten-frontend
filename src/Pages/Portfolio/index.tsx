...
function Portfolio() {
  ...
  const getBalancesFromContract = async (selfC: ethers.Contract, juniorOperatorC: ethers.Contract) => {
  ...
  useEffect(() => {
    if(contractInfo) {
      juniorTokenContract = new ethers.Contract(contractInfo.JUNIOR_TOKEN.address, contractInfo.JUNIOR_TOKEN.ABI, provider?.getSigner());
      seniorTokenContract = new ethers.Contract(contractInfo.SENIOR_TOKEN.address, contractInfo.SENIOR_TOKEN.ABI, provider?.getSigner())
